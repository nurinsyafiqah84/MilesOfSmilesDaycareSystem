		...
		$.confirm({
			...
			buttons: {
				tryAgain: {
					...
					btnClass: 'btn-red',
					action: function(){						
						window.location.replace("viewApplication.php?applicationID=<?php echo $applicationID ?>&reload");
					}
				}
			}
		});		
...
if($_FILES["inputfile"]["error"] == 4) //USER DOES NOT WANT TO UPLOAD FILE!! IT IS OKAY. INSERT OTHER COLUMN
{
	...
	$sqlReply = "INSERT INTO reply (reply, replySender, applicationID) VALUES('".$reply."', '".$username."', '".$applicationID."')";
	if($conn->query($sqlReply) === TRUE)
	{
		...
		$message->setSubject('[MoSD] #'.$applicationID.' New Reply from Administrator');
		...
		$message->setBody('<html>' .
			...
			'Hello, '. $name . 
			'<br><br>'.
			'Your nanny application #'. $applicationID . ' has just received a new reply from our administrator ' . $username . '.' .
			'<br>The snippet of the message is as follows \'' . substr($reply, 0, 150) . ' ...\''.
			'. Please visit the system and view the nanny application to read the remaining message.'.
			'<br><br>'.			
...
else //USER WANTS TO UPLOAD A FILE
{
	...
	if($fileError == 0) //FILE HAS NO PROBLEM 
	{
		if($fileSize < 10000) //FILE DOES NOT EXCEED 10MB
		{
			...
			$fileDestination = '../NannyApplication/uploads/'.$fileNameNew;
